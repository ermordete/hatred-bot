...
async def on_ready():
    ...
    # FIX: Sticky Task war nie gestartet
    if not update_stickies.is_running():
        update_stickies.start()
        print("Sticky update loop started!")
    ...
    activity = discord.Activity(type=discord.ActivityType.playing, name="Combat Warriors")
    ...
    try:
        ...
        if WELCOME_CHANNEL_ID != 0:
            ...
            if welcome_channel:
                ...
                embed.add_field(name="Note", value="Please read the rules and have fun!", inline=False)
...
async def imagelink(interaction: discord.Interaction, image: discord.Attachment = None):
    ...
    if image.content_type not in valid_types:
        error_embed = discord.Embed(
            ...
            description="Erlaubte Typen: PNG, JPG, GIF, WebP",
            color=COLOR_ERROR
        )
        await interaction.response.send_message(embed=error_embed, ephemeral=True)
        return
    ...
    success_embed.add_field(name=f"{BULLET} Link", value=f"`{discord_link}`", inline=False)
    success_embed.add_field(name=f"{BULLET} Größe", value=f"{round(image.size / 1024, 2)} KB", inline=True)
    success_embed.add_field(name=f"{BULLET} Dateityp", value=f"{image.content_type.split('/')[-1].upper()}", inline=True)
...
# ============================================================================
# EMBED MANAGEMENT COMMAND  (FIX: war doppelt definiert - alter /embed entfernt)
# ============================================================================
...
async def embed_command(
    ...
    if action == "create":
        ...
        if name in embeds[guild_id]:
            error = discord.Embed(title="Error", description="Embed existiert bereits!", color=COLOR_ERROR)
            await interaction.response.send_message(embed=error, ephemeral=True)
            return

        embeds[guild_id][name] = {
            "title": title or "Neues Embed",
            "description": description or "Beschreibung",
            "color": color if color else "#191b70",
            "thumbnail": thumbnail,
            "image": image,
            "author_name": None,
            "author_icon": None,
        ...
        error = discord.Embed(title="Invalid Action", description="Erlaubte Actions: create, list, delete, edit, send, preview", color=COLOR_ERROR)
        await interaction.response.send_message(embed=error, ephemeral=True)
...
async def sticky_command(
    ...
    if action == "create":
        if not title or not content:
            error = discord.Embed(title="Error", description="Title und Content erforderlich!", color=COLOR_ERROR)
            await interaction.response.send_message(embed=error, ephemeral=True)
            return
        ...
        if channel_id not in sticky[guild_id]:
            error = discord.Embed(title="Error", description="Keine Sticky Message in diesem Channel!", color=COLOR_ERROR)
            await interaction.response.send_message(embed=error, ephemeral=True)
            return

        sticky[guild_id][channel_id]["enabled"] = False
        save_sticky(sticky)
        success = discord.Embed(title="Sticky Stopped", description="Sticky Message gestoppt!", color=COLOR_SUCCESS)
        await interaction.response.send_message(embed=success, ephemeral=True)
        ...
        save_sticky(sticky)
        success = discord.Embed(title="Sticky Started", description="Sticky Message gestartet!", color=COLOR_SUCCESS)
        await interaction.response.send_message(embed=success, ephemeral=True)
        ...
        save_sticky(sticky)
        success = discord.Embed(title="Sticky Deleted", description="Sticky Message gelöscht!", color=COLOR_SUCCESS)
        await interaction.response.send_message(embed=success, ephemeral=True)

    elif action == "list":
        if not sticky[guild_id]:
            info = discord.Embed(title="Keine Stickies", description="Keine Sticky Messages vorhanden!", color=COLOR_DEFAULT)
            await interaction.response.send_message(embed=info, ephemeral=True)
            return

        list_embed = discord.Embed(title="Alle Sticky Messages", color=COLOR_DEFAULT)
        ...
        error = discord.Embed(title="Invalid Action", description="Erlaubte Actions: create, stop, start, delete, list", color=COLOR_ERROR)
        await interaction.response.send_message(embed=error, ephemeral=True)
...
@app_commands.describe(
    action="create, close, delete, list",
...
async def ticket_command(
    ...
    if action == "create":
        ...
        try:
            ...
            # FIX: Berechtigungen für den Ticket-Ersteller setzen
            overwrites = {
            ...
            # Support Role Zugriff geben falls konfiguriert
            if TICKET_SUPPORT_ROLE_ID != 0:
            ...
            embed.add_field(name=f"{BULLET} Grund", value=reason, inline=False)
            embed.add_field(name=f"{BULLET} Status", value="🟢 Offen", inline=False)
            ...
            error = discord.Embed(title="Error", description=f"Fehler beim Erstellen des Tickets: {str(e)}", color=COLOR_ERROR)
            await interaction.response.send_message(embed=error, ephemeral=True)
        ...
        if channel_id not in tickets[guild_id]:
            error = discord.Embed(title="Error", description="Das ist kein Ticket Channel!", color=COLOR_ERROR)
            await interaction.response.send_message(embed=error, ephemeral=True)
            return
        ...
        closed_embed = discord.Embed(title="Ticket Closed", description="Dieses Ticket wurde geschlossen!", color=COLOR_SUCCESS)
        await interaction.channel.send(embed=closed_embed)

        success = discord.Embed(title="Closed", description="Ticket geschlossen!", color=COLOR_SUCCESS)
        await interaction.response.send_message(embed=success, ephemeral=True)
        ...
        if not tickets[guild_id]:
            info = discord.Embed(title="Keine Tickets", description="Keine Tickets vorhanden!", color=COLOR_DEFAULT)
            await interaction.response.send_message(embed=info, ephemeral=True)
            return

        list_embed = discord.Embed(title="Alle Tickets", color=COLOR_DEFAULT)
        ...
        error = discord.Embed(title="Invalid Action", description="Erlaubte Actions: create, close, delete, list", color=COLOR_ERROR)
        await interaction.response.send_message(embed=error, ephemeral=True)
...
async def membercount(interaction: discord.Interaction):
    ...
    embed.add_field(name="Info", value="All active members in the Hatred server", inline=False)
    await interaction.response.send_message(embed=embed)

@bot.tree.command(name="rules", description="Show server rules")
async def rules(interaction: discord.Interaction):
    embed = discord.Embed(title=f"──────── {SERVER_TAG} Rules ────────", color=COLOR_DEFAULT)
    embed.add_field(name=f"{BULLET} Respect", value="Treat all members with respect and kindness", inline=False)
    embed.add_field(name=f"{BULLET} No Spam", value="Do not spam messages or content", inline=False)
    embed.add_field(name=f"{BULLET} NSFW Content", value="Keep NSFW content to designated channels only", inline=False)
    embed.add_field(name=f"{BULLET} No Cheating", value="Cheating and exploits are strictly forbidden", inline=False)
    embed.add_field(name=f"{BULLET} No Discrimination", value="Discrimination is not tolerated", inline=False)
    await interaction.response.send_message(embed=embed)
...
async def help_command(interaction: discord.Interaction):
    embed = discord.Embed(
        title=f"──────── {SERVER_TAG} Commands ────────",
        description="Here are all available commands:",
        color=COLOR_DEFAULT
    )

    embed.add_field(
        name=f"{BULLET} Admin/Mod Commands",
        value="`/embed` - Embed Management\n"
              ...
              "`/imagelink` - Upload image and get Discord link",
        inline=False
    )

    embed.add_field(
        name=f"{BULLET} Birthday Commands",
...
async def birthday(interaction: discord.Interaction, action: str, date: str = None):
    # FIX: Nur prüfen wenn BIRTHDAY_ROLE_ID konfiguriert ist
    ...
    if action == "set":
        ...
        if not birthdays:
            embed = discord.Embed(title="Birthday List", description="No birthdays registered yet", color=COLOR_DEFAULT)
            await interaction.response.send_message(embed=embed)
            return

        sorted_birthdays = sorted(birthdays.items(), key=lambda x: x[1]['date'])
        ...
        for uid, data in sorted_birthdays:
            embed.add_field(
        ...
        if user_id not in birthdays:
            embed = discord.Embed(title="No Birthday Set", description="You haven't set your birthday yet", color=COLOR_ERROR)
            await interaction.response.send_message(embed=embed, ephemeral=True)
            return
        ...
        embed = discord.Embed(title="Birthday Removed", description="Your birthday has been removed", color=COLOR_SUCCESS)
        await interaction.response.send_message(embed=embed, ephemeral=True)

    elif action == "test":
        if BIRTHDAY_CHANNEL_ID == 0:
            embed = discord.Embed(title="Error", description="Birthday channel is not configured", color=COLOR_ERROR)
            await interaction.response.send_message(embed=embed, ephemeral=True)
            return
        ...
        try:
            birthday_channel = bot.get_channel(BIRTHDAY_CHANNEL_ID)
            if not birthday_channel:
                embed = discord.Embed(title="Error", description="Birthday channel not found", color=COLOR_ERROR)
                await interaction.response.send_message(embed=embed, ephemeral=True)
                return
            ...
            error = discord.Embed(title="Error", description=f"Failed to send test message: {e}", color=COLOR_ERROR)
            await interaction.response.send_message(embed=error, ephemeral=True)

    else:
        embed = discord.Embed(title="Invalid Action", description="Use: set, view, list, remove, or test", color=COLOR_ERROR)
        await interaction.response.send_message(embed=embed, ephemeral=True)
...
async def announce(interaction: discord.Interaction, channel: discord.TextChannel, message: str):
    if not interaction.user.guild_permissions.manage_messages:
        embed = discord.Embed(title="Permission Denied", description="You need 'Manage Messages' permission", color=COLOR_ERROR)
        await interaction.response.send_message(embed=embed, ephemeral=True)
        return

    try:
        embed = discord.Embed(title=SEPARATOR_FANCY, description=message, color=COLOR_DEFAULT)
        embed.set_footer(text=f"{SERVER_TAG} Server")
        await channel.send(embed=embed)

        confirm = discord.Embed(title="Announcement Sent", description=f"Message sent to {channel.mention}!", color=COLOR_SUCCESS)
        await interaction.response.send_message(embed=confirm, ephemeral=True)
    except Exception as e:
        error = discord.Embed(title="Error", description=f"Failed to send: {e}", color=COLOR_ERROR)
        await interaction.response.send_message(embed=error, ephemeral=True)
...
async def warn(interaction: discord.Interaction, member: discord.Member, reason: str = "No reason provided"):
    if not interaction.user.guild_permissions.moderate_members:
        embed = discord.Embed(title="Permission Denied", description="You need moderation permissions", color=COLOR_ERROR)
        await interaction.response.send_message(embed=embed, ephemeral=True)
        return
    ...
    embed = discord.Embed(title="Member Warned", description=f"{member.mention} has been warned", color=COLOR_ERROR)
    embed.add_field(name=f"{BULLET} Reason", value=reason, inline=False)
    embed.add_field(name=f"{BULLET} Total Warnings", value=warn_count, inline=False)
...
async def kick(interaction: discord.Interaction, member: discord.Member, reason: str = "No reason provided"):
    if not interaction.user.guild_permissions.kick_members:
        embed = discord.Embed(title="Permission Denied", description="You need kick permissions", color=COLOR_ERROR)
        await interaction.response.send_message(embed=embed, ephemeral=True)
        return

    try:
        await member.kick(reason=reason)
        embed = discord.Embed(title="Member Kicked", description=f"{member.mention} has been kicked", color=COLOR_SUCCESS)
        embed.add_field(name=f"{BULLET} Reason", value=reason, inline=False)
        embed.set_footer(text=f"Kicked by {interaction.user.name}")
        await interaction.response.send_message(embed=embed)
    except Exception as e:
        error = discord.Embed(title="Error", description=f"Could not kick member: {e}", color=COLOR_ERROR)
        await interaction.response.send_message(embed=error, ephemeral=True)
...
async def ban(interaction: discord.Interaction, member: discord.Member, reason: str = "No reason provided"):
    if not interaction.user.guild_permissions.ban_members:
        embed = discord.Embed(title="Permission Denied", description="You need ban permissions", color=COLOR_ERROR)
        await interaction.response.send_message(embed=embed, ephemeral=True)
        return

    try:
        await member.ban(reason=reason)
        embed = discord.Embed(title="Member Banned", description=f"{member.mention} has been banned", color=COLOR_SUCCESS)
        embed.add_field(name=f"{BULLET} Reason", value=reason, inline=False)
        embed.set_footer(text=f"Banned by {interaction.user.name}")
        await interaction.response.send_message(embed=embed)
    except Exception as e:
        error = discord.Embed(title="Error", description=f"Could not ban member: {e}", color=COLOR_ERROR)
        await interaction.response.send_message(embed=error, ephemeral=True)
...
class RoleButton(discord.ui.View):
    ...
    async def role_button(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        if not role:
            embed = discord.Embed(title="Error", description="Role not found", color=COLOR_ERROR)
            await interaction.response.send_message(embed=embed, ephemeral=True)
            return
        ...
        try:
            if role in interaction.user.roles:
                ...
                embed = discord.Embed(title="Role Removed", description=f"{role.mention} has been removed", color=COLOR_SUCCESS)
            else:
                await interaction.user.add_roles(role)
                embed = discord.Embed(title="Role Added", description=f"{role.mention} has been added", color=COLOR_SUCCESS)

            await interaction.response.send_message(embed=embed, ephemeral=True)
        except Exception as e:
            error = discord.Embed(title="Error", description=f"Could not assign role: {e}", color=COLOR_ERROR)
            await interaction.response.send_message(embed=error, ephemeral=True)

@bot.tree.command(name="role_button", description="Create a button to get a role")
@app_commands.describe(role="The role to assign", title="Button label (what it should say)")
async def role_button(interaction: discord.Interaction, role: discord.Role, title: str = "Get Role"):
    if not interaction.user.guild_permissions.manage_roles:
        embed = discord.Embed(title="Permission Denied", description="You need role management permissions", color=COLOR_ERROR)
        await interaction.response.send_message(embed=embed, ephemeral=True)
        return
    ...
    try:
        ...
        error = discord.Embed(title="Error", description=f"Could not create button: {e}", color=COLOR_ERROR)
        await interaction.response.send_message(embed=error, ephemeral=True)

@bot.tree.command(name="welcome_setup", description="Set welcome role for new members")
...
async def welcome_setup(interaction: discord.Interaction, role: discord.Role):
    if not interaction.user.guild_permissions.manage_roles:
        embed = discord.Embed(title="Permission Denied", description="You need Administrator permissions", color=COLOR_ERROR)
        await interaction.response.send_message(embed=embed, ephemeral=True)
        return
    ...
    try:
        ...
        error = discord.Embed(title="Error", description=f"Error: {e}", color=COLOR_ERROR)
        await interaction.response.send_message(embed=error, ephemeral=True)
...
async def on_app_command_error(interaction: discord.Interaction, error: app_commands.AppCommandError):
    if isinstance(error, app_commands.MissingPermissions):
        embed = discord.Embed(title="Permission Denied", description="You don't have permission!", color=COLOR_ERROR)
        await interaction.response.send_message(embed=embed, ephemeral=True)
    else:
        embed = discord.Embed(title="Error Occurred", description=str(error), color=COLOR_ERROR)
        # FIX: Prüfen ob bereits geantwortet wurde
